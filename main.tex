...
% Multiclass classification (>2) VS Binary classification (2)
...
%talk about sound (nearly) always coming in as a mixture of different sounds (possibly from different sources).

%talk about sound being waves of air pressure, of which the vibrations are detected by a sensor (microphone) and converted to electrical voltage by a "sound-card" to perform the analog to digital conversion (ADC) process. In the end a discrete-sampled (digital) waveform (which can be processed and saved on a computer) is obtained.

%praat oor resolution van mics i.t.v. Bit Depth (vir quantization op die y-as (amplitude)) en ook i.t.v. Sampling Rate/Frequency (en noem ook die Nyquist frequency is 44100/2 = 22050 en dus voldoende weens human hearing range wat stop by 20k)
...
%the digital waveform is quantized in: time (44 100 Hz sampling rate of mic) and in amplitude (16 bit d.w.s 2^16 uniquely identifiable frequencies)

%mention how it's safest (by avoiding artifacts and so on) to use an uncompressed format (i.e. WAV (and not MP3?)) for the audio samples. Basically ganit net oor audio files wat redelik groot is, en waarvoor daar al baie compression methods design is e.g. MP3 en al doen dit 'n goeie job is dit ongelukkig ook redelik lossy)

%verduidelik van SNR (of SQNR, Signal-to-Quantization-Noise Ratio) en werk hom uit vir 'n bit depth van 16 as: SQNR(16) ~= 96 dB

%mention that the 4s samples will be divided into a few different windows/segments (e.g. four 1s windows) -> to reduce the complexity of the machine learning algorithm. OVERLAP may be introduced with this windows (e.g. 50% overlap means the neural network will be considering most of the audio sample twice). OF discard MFCC's wat nie 'n spesifieke grootte het nie.
...
%mention AutoEncoders

%mention Audio Embeddings (inspired by text embeddings), (uses a CNN "under the hood?") RECOMMENDED TO START HERE

%praat oor (EN APPLY) Data Augmentation:
...
%mention OVERFITTING en doen die volgende goed om dit te fix:
%1. simpler architecture (remove layers, decrease # of neurons)
%en als hieronder ook...

%TODO mention AND APPLY dropout (first implemented by AlexNet in 2012) en vergelyk verskillende "dropout probabilities"
...
%TODO praat oor Regularization (L1 en L2 methods) om 'n penalty te add "to punish large weights" (in hierdie geval sal ek L2 gebruik)

%TODO verduidelik softmax en hoe dit gesien kan word as die final activation function vir die neural net

%TODO praat oor CNN's en hoe hulle gebruik word vir CV; onthou van ZERO PADDING; AND THE WEIGHTS OF THE KERNELS ARE THE VALUES THAT ARE BEING LEARNED BY THE NETWORK

%Architectural decisions for CNN's:
%1. Grid size (of the kernel) --> usually(?) an odd value (so that it has a centre pixel)
%2. Stride ("step size" while applying kernel to spectrogram/image)
%3. Depth (het te doen met verskillende channels e.g. RGB; nie rerag van toepassing hier nie tensy ons die ander channel van die WAV file ook in berekening bring; dan sou jy essentially nog 'n dimensie by die kernel se blokkie gevoeg het... Vir my use-case is die depth=1)
%4. Number of kernels ==> Elke kernel output 'n enkele 2D array. Jy kan meer kernels bysit dan't jy soveel meer 2D arrays as output vir hierdie convolutional layer.
%5. Pooling schema (e.g. max/average pooling; grootte vir groepie wat saamgepool word, grootte van strides/steps)
...
%TODO include verskillende tipes optimizers (e.g. Adam) en learning rates en loss function (e.g. sparse_categorical_crossentropy) en batching (e.g. mini-batches tussen 16 en 128 samples) vir stochastic gradient descent during training.

%TODO include verskillende activation functions (sigmoid vs relu)
%relu laat mens toe om vinniger te train en dit vermy ook vanishing gradient
